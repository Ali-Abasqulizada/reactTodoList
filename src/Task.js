...
export default function Task({ name, done, onToggle, onTrash, onRename }) {
    ...
    return (
        <div className={"task " + (done ? "done" : "")}>
            ...
            {editMode && (
                <form className="update" onSubmit={e => {e.preventDefault(); setEditMode(false)}}>
                    <input className="update-input" type="text" value={name} onChange={e => onRename(e.target.value)} />
                </form>
            )}
